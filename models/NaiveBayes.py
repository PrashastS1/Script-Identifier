...
GAP_DIM = None    # Will need to be set before running or handled dynamically
...
if __name__ == '__main__':
    ...
    # --- Grid Search for PCA and Naive Bayes ---
    logger.info("Starting GridSearchCV for PCA and Naive Bayes hyperparameters...")
    ...
    for n_components in PARAM_GRID['n_components']:
        for whiten in PARAM_GRID['whiten']:
            ...
            logger.info(f"Best NB parameters for PCA (n_components={n_components}, whiten={whiten}): {grid_search.best_params_}")
    ...
    plot_confusion_matrix(
        ...
        filename=f"confusion_matrix_{BACKBONE}_nb.png"
    )

    plot_accuracy_vs_hyperparameters(
        all_grid_results[0], 'var_smoothing',
        title=f"Accuracy vs. var_smoothing (Backbone: {BACKBONE})",
        filename=f"accuracy_vs_var_smoothing_{BACKBONE}_nb.png"
    )
