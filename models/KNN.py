...
# # Tune KNN hyperparameters and plot elbow curve
# print("Tuning KNN hyperparameters and generating elbow curve...", flush=True)
# k_values = range(1, 12, 2)  # Test k from 1 to 11
# f1_scores = []
# for k in k_values:
#     model = KNeighborsClassifier(n_neighbors=k, weights='distance', metric=cosine_distances, n_jobs=-1)
#     scores = cross_val_score(model, x_train, y_train, cv=5, scoring='f1_macro')
#     mean_score = np.mean(scores)
...
# # plt.title("Elbow Curve for KNN (F1-Macro vs. k)")
# # plt.grid(True)
# # timestamp = datetime.datetime.now().strftime("%Y%m%d_%H%M%S")
# # elbow_path = os.path.join(script_dir, "plots", f"{selected_lang}_elbow_curve_{timestamp}.png")
# # os.makedirs(os.path.dirname(elbow_path), exist_ok=True)
# # plt.savefig(elbow_path)
# # plt.show()
# # print(f"Elbow curve saved at: {elbow_path}", flush=True)

# # Select best k (highest F1-macro score)
# best_k = k_values[np.argmax(f1_scores)]
# print(f"Best k based on elbow curve: {best_k}", flush=True)

# # Train KNN model with best k
# print(f"Training KNN with best k={best_k}...", flush=True)
# model = KNeighborsClassifier(n_neighbors=best_k, weights='distance', metric=cosine_distances, n_jobs=-1)
# model.fit(x_train, y_train)
# print("KNN model training completed.", flush=True)
...
# # Try different thresholds to optimize F1-score for Class 1
# best_threshold = 0.7
# best_f1_class1 = 0
...
#     print(f"Threshold={threshold:.2f}, F1-score for Class 1: {f1_class1:.2f}", flush=True)
#     if f1_class1 > best_f1_class1:
#         best_f1_class1 = f1_class1
#         best_threshold = threshold

# # Use the best threshold
# print(f"Using best threshold={best_threshold:.2f}...", flush=True)
...
# # plt.title(f"Confusion Matrix for {selected_lang} (KNN + HOG)")
...
# #     timestamp = datetime.datetime.now().strftime("%Y%m%d_%H%M%S")
# #     filename = f"{language}_knn_hog_decision_boundary_{timestamp}.png"
# #     plot_path = os.path.join(plots_dir, filename)
# #     plt.savefig(plot_path)
# #     plt.show()
...
# # save_decision_boundary_plot(x_train, y_train, model, selected_lang)
# # save_scatter_plot(x_test, y_test, selected_lang)
