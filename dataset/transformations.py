...
class LanguageRecognitionTransforms:
    ...
    def get_transforms(backbone_type: str = None, phase: str = 'train', img_size: int = 224):
        """
        Args:
            backbone_type: None (custom), 'resnet50', 'vgg', 'vit', 'swin', 'beit'
            phase: 'train' or 'test'
        ...
        norms = {
            ...
            'vit': {'mean': [0.5, 0.5, 0.5], 'std': [0.5, 0.5, 0.5]},  # For custom ViT
        ...
        base_params = {
            ...
            'mask_value': 255,  # White background assumption
        ...
        if phase == 'test':
            return A.Compose([
                ...
                A.Normalize(**norms['imagenet' if backbone_type in ['resnet50', 'vgg', 'vit'] else 'default']),
                ToTensorV2()
            ])


        transforms = []
        crop_size = int(img_size * 1.2)  # 20% buffer
        ...
        # transformations based on backbone type
        if not backbone_type or backbone_type == 'custom':
            transforms.extend([
                ...
                A.Affine(
                    ...
                    scale=(0.9, 1.1),  # Matches scale_limit=0.1 (1.0 ± 0.1)
                    rotate=(-30, 30),
                    shear=0,  # Disable shear for text preservation
                    interpolation=cv2.INTER_CUBIC,  # Better for document images
                    border_mode=cv2.BORDER_CONSTANT,
                    fill=255,  # White background for text consistency
                    fit_output=False,
                    keep_ratio=False,  # Required for exact ShiftScaleRotate equivalence
                    p=0.5
                ),
            ...
            transforms.extend([
                ...
                A.Normalize(**norms['imagenet']),  # All official ViTs use ImageNet stats
                ToTensorV2()
            ])
